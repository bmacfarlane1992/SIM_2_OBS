...
(b) Generate an octree with locational data from particle array
(a) Generate an octree as above, populating grid bins with SPH densities

Program ***HEAVILY*** uses modules and functions from HYPERION code, stored
in hyperion.importers.sph, hyperion.importers.util and hyperion.importers.grid
respectively.
...
def refine(x, y, z, dx, dy, dz, px, py, pz, sigma, mass, levels_remaining, \
  stopping_criterion):

    if stopping_criterion(x, y, z, dx, dy, dz, px, py, pz, sigma) or \
      (levels_remaining == 0):
        return [False]

    b_all = [True]

    px_pos = px > x ; py_pos = py > y ; pz_pos = pz > z
    ...
    for zcomp, zsub in ((~pz_pos, z-dz*0.5), (pz_pos, z + dz * 0.5)):
        for ycomp, ysub in ((~py_pos, y-dy*0.5), (py_pos, y + dy * 0.5)):
            for xcomp, xsub in ((~px_pos, x-dx*0.5), (px_pos, x + dx * 0.5)):
                ...
                b = refine(xsub, ysub, zsub, dx * 0.5, dy * 0.5, dz * 0.5, \
                  px[keep], py[keep], pz[keep], sigma[keep], mass[keep], \
                  levels_remaining - 1, stopping_criterion)
...
def construct_octree(x, y, z, dx, dy, dz, px, py, pz, sigma, mass, \
  n_levels=None, stopping_criterion=None):

    '''
    Construct Octree from SPH particles.

    This will set up the octree, and then compute the densities. It is
    equivalent to calling:
    compute_octree_geometry(), then
    compute_octree_density
    ...
    mass : np.ndarray
        The mass of them SPH particles
    n_levels : int
        Max. number of levels in the octree
    stopping_criterion : func
        Function defined to determine whether cell is refined

    Returns
    -------
    grid : octree_grid.OctreeGrid
        The octree grid with the densities set
    '''

    # Set up geometrical OctreeGrid

    octree = compute_octree_geometry(x, y, z, dx, dy, dz, px, py, pz, sigma, \
      mass, n_levels=n_levels, stopping_criterion=stopping_criterion)

    # Compute densities and add to octree

    compute_octree_densities(octree, px, py, pz, sigma, mass)
...
def compute_octree_geometry(x, y, z, dx, dy, dz, px, py, pz, sigma, mass, \
  n_levels, stopping_criterion):

    '''
    Generate an Octree grid from SPH particles, appending location data to
    octree.
    ...
    Returns
    -------
    grid : octree_gid.OctreeGrid
        The octree grid with locational adata
    '''

    # Compute refined array

    refined = refine(x, y, z, dx, dy, dz, px, py, pz, sigma, mass, \
      n_levels, stopping_criterion)
    ...
    # Compute and append octree with location data of grid points

    xmin, xmax, ymin, ymax, zmin, zmax = octree.limits
    xwid = xmax - xmin ; xcen = xmin + (xwid / 2.0)
    ywid = ymax - ymin ; ycen = ymin + (ywid / 2.0)
    zwid = zmax - zmin ; zcen = zmin + (zwid / 2.0)

    octree["xcen"] = [] ; octree["xcen"].append(xcen)
    octree["ycen"] = [] ; octree["ycen"].append(ycen)
    octree["zcen"] = [] ; octree["zcen"].append(zcen)
    octree["xwid"] = [] ; octree["xwid"].append(xwid)
    octree["ywid"] = [] ; octree["ywid"].append(ywid)
    octree["zwid"] = [] ; octree["zwid"].append(zwid)

    return octree

### ------------------------------------------------------------------------ ###

def compute_octree_densities(octree, px, py, pz, sigma, mass):

    '''
    Compute the density of the cells in an Octree,
    then append octree with this data
    ...
    # If openmp, compute densities using nthreads CPU count
    ...
    # Split them up for mutliprocessing
    ...
    # For parent cells, set density to zero

    density[np.where(octree.refined)] = 0.0

    # Normalize densities by volume

    density = density / ( (xmax - xmin) * (ymax - ymin) * (zmax - zmin) )

    octree["density"] = [] ; octree["density"].append(density)
